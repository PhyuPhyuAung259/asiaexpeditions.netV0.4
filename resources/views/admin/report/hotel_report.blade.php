...
<table class="table" id="roomrate">
    @if ($priceType == 'contract')
            ...
            <td colspan="2">
                <address>
                    <p><b>Hotel Name :</b> {{ $supplier->supplier_name }}<br>
                        ...
                        <b>Website :</b> {{ $supplier->supplier_website }}
                    </p>
                </address>
        ...
        @if ($supplier->supplier_term_condition)
            <tr class="custom-border">
                <td colspan="2">
                    <strong>Terms & Conditions</strong>
                    {!! $supplier->supplier_term_condition !!}
                </td>
            </tr>
        @endif
        ...
        @foreach ($supplier->room as $room)
            ...
            @foreach ($getRate as $rate)
                <tr>
                    ...
                    @if ($priceType != 'selling')
                        <td class="text-right">{{ Content::money($rate->nsingle) }} <small
                                class="pcolor">{{ $rate->nsingle > 0 ? Content::currency() : '' }}</small></td>
                        <td class="text-right">{{ Content::money($rate->ntwin) }} <small
                                class="pcolor">{{ $rate->ntwin > 0 ? Content::currency() : '' }}</small></td>
                        <td class="text-right">{{ Content::money($rate->ndbl_price) }} <small
                                class="pcolor">{{ $rate->ndbl_price > 0 ? Content::currency() : '' }}</small></td>
                        <td class="text-right">{{ Content::money($rate->nextra) }} <small
                                class="pcolor">{{ $rate->nextra > 0 ? Content::currency() : '' }}</small></td>
                        <td class="text-right">{{ Content::money($rate->nchexbed) }} <small
                                class="pcolor">{{ $rate->nchexbed > 0 ? Content::currency() : '' }}</small></td>
                    @endif
                </tr>
            @endforeach
        @endforeach
