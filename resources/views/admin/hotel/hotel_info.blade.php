    ...
    <section class="content"> 
      <div class="row">
        ...
        <section class="col-lg-12">
          <h3 class="border">Hotel Info <span class="fa fa-angle-double-right"></span> <a href="#" class="btn btn-default btn-sm btnEditRoom" data-toggle="modal" data-target="#myModal">Add New </a></h3>
          <table class="datatable table table-hover table-striped">
            ...
            <tbody>
              @foreach($hotelinfo as $rom)
              <tr>
                <td>{{$rom->name}}</td>        					                  
    						<td class="text-center"> 
                    <a class="btnEditRoom" href="#" title="Hotel Info" data-toggle="modal" data-id="{{$rom->id}}" data-name="{{$rom->name}}" data-target="#myModal">
                      <label class="icon-list ic_edit"></label>
                    </a>	                        
                    &nbsp;&nbsp;
                    <a href="javascript:void(0)" class="RemoveHotelRate" data-type="hotel_info" data-id="{{$rom->id}}" title="Remove this?">
...
<div class="modal fade" id="myModal" role="dialog" data-backdrop="static" data-keyboard="true">
  <div class="modal-dialog modal-md">
    <form method="POST" action="{{route('addHotelinfo')}}">
      <div class="modal-content">       
        <input type="hidden" name="eid" id="roomid"> 
        <div class="modal-header" style="padding: 5px 13px;">
          <button type="button" class="close" data-dismiss="modal">&times;</button>
          <h4 class="modal-title"><strong>Add New Hotel Info</strong></h4>
        </div>
