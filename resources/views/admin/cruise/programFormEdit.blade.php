    ...
    <div class="content-wrapper">
      <section class="content"> 
        <div class="row">
          ...
          <form method="POST" action="{{route('updateCrprogram')}}">
              {{csrf_field()}}
              <input type="hidden" name="eid" value="{{$crpro->id}}">
                  ...
                  <div class="row">
                    ...
                    <div class="col-md-6 col-xs-12">
                      <div class="form-group">
                        ...
                        <input type="text" name="program_name" class="form-control" value="{{$crpro->program_name}}" required>
                      </div> 
                    </div>
                  </div>  
                  <div class="form-group">
                    <label>Hightlights</label>                    
                    <textarea class="form-control" name="program_hight" rows="6">{!!$crpro->program_highlight!!}</textarea>
                  </div>
                  <div class="form-group">
                    <label>Remarks</label>
                    <textarea class="form-control" name="program_remark" rows="6">{!! $crpro->program_remark !!}</textarea>             
                  </div>
                  <div class="form-group">
                    <label>Description</label>
                    <script src="{{asset('adminlte/editor/tinymce.min.js')}}"></script>
                    <textarea class="form-control my-editor" name="program_desc" rows="6">{!!$crpro->program_intro!!}</textarea>             
                  </div>
                  <div class="form-group">
                    ...
                    @foreach(App\CrCabin::where('status', 1)->orderBy('name', 'ASC')->get() as $key=>$cabin)
                      <div>                        
                        <span style="position:relative;top:3px;padding-right:3px;">
                          <i data-id="{{$key}}" class="fa {{in_array($cabin->id, explode(',', $dataid)) ? 'fa-check-square' :'fa-square-o'}} nocheck"></i>
                          <input style="opacity: 0;" class="choose-option" type="checkbox" name="crcabin[]" value="{{$cabin->id}}" id="{{$key}}" {{in_array($cabin->id, explode(',', $dataid)) ? 'checked':''}}>
                        </span>
                  ...
                  <div class="panel-body">               
                    <select class="form-control" name="program_dest">
                      @foreach(App\Province::where(['country_id'=>$supplier->country_id, 'province_status'=> 1])->orderBy('province_name', 'ASC')->get() as $key => $pro)
                        <option value="{{$pro->id}}" {{$pro->id == $crpro->province_id ? 'selected':''}}>{{$pro->province_name}}</option>
                      @endforeach
                    </select>
                  </div>                  
                </div>
