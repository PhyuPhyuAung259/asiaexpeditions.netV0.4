...
@section('content')
	...
	<div class="container-fluid">
			...
			@if(isset($project->project_number))
				...
				<table class="table table-borderd" border="1">
					...
					@if(!empty($totalRevenue))
						...
						<tr>
							<td>{{{ Content::dateformat($project->project_start) or ' '}}} -> {{{ Content::dateformat($project->project_end) or ''}}}</td>
							...
							<td class="text-center" colspan="2">
								<?php
			      				...
			      				//	dd($color);
		      						?>
	      						<span class="pull-left" {{$color}}> {{{ $totalRevenue == $accTransaction->sum('debit') ? Content::money($totalRevenue) : Content::money($totalRevenue - $accTransaction->sum('debit')) or ''}}}</span>
	      						@if( $proJournal['book_amount'] > 0 ) 	  								
    	  								<span class="badge badge-light pull-right">{{{ Content::money($accTransaction->sum('debit')) or ''}}}</span>
				...
				//	dd($hotelBook);?>
					@if($hotelBook->count() > 0)
						...
						<tr class="container_hotel">
							...
							<?php $hotelAmount = $hotel->net_amount;
								$totalHotel = $totalHotel + $hotelAmount;
	      						...
	      						?>
	          					<span class="pull-left" {{$color}}>{{{ $hotelAmount == $hTransaction->sum('credit') ? Content::money($hotelAmount) : Content::money($hotelAmount - $hTransaction->sum('credit')) or ''}}}</span>	
 
								@if( !empty($HproJournal['book_amount']) )									
									<span class="badge badge-light pull-right">{{ Content::money($hTransaction->sum('credit') ) }}</span>
						...
						@endforeach
						<tr>
							<td colspan="11" class="text-right"><h5><strong>Total: {{{ Content::money($totalHotel) or ''}}} {{Content::currency()}} </strong></h5></td>
						</tr>				
					@endif

					<?php $flightBook = App\Booking::flightBook($project->project_number)->get(); dd($flightBook); ?>
					...
					<tr class="flight">
						...
						@foreach($flightBook as $fl)
							...
							<tr class="container_flight">
								<td>{{{ Content::dateformat($fl->book_checkin) or ''}}}</td>
								<?php
								...
								<td class="text-right" {{$color}}>{{{ $fl->book_namount == $fSaction->sum('credit') ? Content::money($fl->book_namount) :   Content::money($fl->book_namount - $fSaction->sum('credit') ) or '' }}}</td>
								<td class="text-right">{{{ Content::money($fl->book_nkprice) or '' }}}</td>
								<td class="text-right">
									<span class="pull-left" {{$colork}}>{{{ $fl->book_nkamount == $fSaction->sum('kcredit') ? Content::money($fl->book_nkamount) : Content::money($fl->book_nkamount - $fSaction->sum('kcredit')) or ''}}}</span>
					...
					@if($golfBook->count() > 0)
						...
						?>
							<tr class="container_golf">
								<td>{{{ Content::dateformat($gf->book_checkin) or '' }}}</td>
		...
		{{{ $gf->book_namount == $Gsaction->sum('credit') ? Content::money($gf->book_namount) : Content::money($gf->book_namount - $Gsaction->sum('credit')) or ''}}}</td>
									<td class="text-right">{{{ Content::money($gf->book_nkprice) or '' }}}</td>
									<td class="text-center" style="width: 17%;">
										<span class="pull-left" {{$colork}}>
		{{{ $gf->book_nkamount == $Gsaction->sum('kcredit') ? Content::money($gf->book_nkamount) : Content::money($gf->book_nkamount - $Gsaction->sum('kcredit')) or ''}}}</span>
					...
					@if($cruiseBook->count() > 0)
						...
						<tr class="container_river-cruise">
							<td>{{{ Content::dateformat($crp->checkin) or '' }}} -> {{{ Content::dateformat($crp->checkout) or '' }}} </td>
							...
							<td class="text-right">							
								<span class="pull-left" {{$color}}> {{{ $crp->net_amount == $rsaction->sum('credit') ? Content::money($crp->net_amount) : Content::money($crp->net_amount - $rsaction->sum('credit')) or '' }}}</span>
						...
						<tr>
							<td colspan="11" class="text-right"><h5><strong>
								Total {{ Content::currency()}} : {{{ Content::money($totalCruise) or '' }}}</strong></h5>
							</td>
						</tr>
					@endif
					<!-- Transport Start-->
					...
					@if( $tranBook->count() > 0)
						...
						<tr class="container_transport">
							<td>{{{ Content::dateformat($tran->book_checkin) or '' }}}</td>
			                ...
			                <td class="text-right" {{$color}}>{{{ $btran['price'] == $TransacTran->sum('credit') ? Content::money($btran['price']) : Content::money($btran['price'] - $TransacTran->sum('credit')) or '' }}}</td>
		                  	<td class="text-right" style="width: 17%;">
		                  		<span {{$colork}}>{{{ Content::money($btran['kprice'] - $TransacTran->sum('kcredit')) or '' }}}</span>
	              		
	      					@if($TranJournal['book_amount'] > 0 || $TranJournal['book_kamount'] > 0)
	      						<span class="badge badge-light pull-right">{{{ Content::money($TransacTran->sum('credit')) or '' }}}</span>
	          				...
	          				@else          				
	          					@if($TranAmount > 0)
	              				<span data-toggle="modal" data-target="#loadProjectConfirm" class="btn btn-success btn-xs AddToACcount text-right hidden-print pull-right" 
	      							data-type="7" data-kamount="{{$btran['kprice']}}" 
					...
					@if($guideBook->count() > 0)
						...
						@foreach($guideBook as $tran)
							...
							<tr class="container_guide">
								<td><span class="hidden-print" style="position: relative;top:2px;"></span>{{{ Content::dateformat($tran->book_checkin) or ''}}}</td>
								<!-- <td>{{$pro->province_name}}</td>          -->
								<td colspan="4">{{{ $tran->tour_name or ''}}} &nbsp;<i>[ {{{ $pro->province_name or '' }}} ]</i></td>     
								<td colspan="2">{{{ $bg->service->title or '' }}}</td>
								<td>{{{ $bg->supplier->supplier_name or ''}}} </td>
								<td>{{{$bg->language->name or ''}}}</td>
								<td class="text-right" {{$color}}> {{{ $bg['price'] == $GAccTran->sum('credit') ? Content::money($bg['price']) : Content::money($bg['price'] - $GAccTran->sum('credit')) or '' }}}</td>
								<td class="text-right" style="vertical-align: middle;">
									
									<span {{$colork}}>{{{ $bg['kprice'] == $GAccTran->sum('kcredit') ? Content::money($bg['kprice']) : Content::money($bg['kprice'] - $GAccTran->sum('kcredit')) or '' }}}</span>
					...
					$restBook = App\BookRestaurant::where('project_number', $project->project_number)->orderBy('start_date')->get();?>
						@if($restBook->count() > 0)
							...
							<tr class="container_restaurant">
								<td>{{{ Content::dateformat($rest->start_date) or '' }}}</td>
				                ...
				                <td class="text-right" {{$color}}>{{{ $rest->amount == $RestAccTran->sum('credit') ? Content::money($rest->amount) : Content::money($rest->amount - $RestAccTran->sum('credit') ) or '' }}} </td>
				                <td class="text-right">{{{ Content::money($rest->kprice) or '' }}}</td>
			                  	<td class="text-left">
			                  		<span>{{{ Content::money($rest->kamount - $RestAccTran->sum('kcredit') ) or ''}}}</span>
					...
					$EntranceBook = App\BookEntrance::where('project_number', $project->project_number)->orderBy('start_date', 'ASC')->get(); ?>
						...
						@if($EntranceBook->count() > 0)
							...
							@foreach($EntranceBook as $ent)
								...
								?>
								<tr>
									<td>{{{ Content::dateformat($ent->start_date) or '' }}}</td>
					                ...
					                <td class="text-right" {{$color}}>{{{ $entAmount == $EntAccTran->sum('credit') ? Content::money($entAmount) : Content::money($entAmount - $EntAccTran->sum('credit')) or '' }}}</td>
					                <td class="text-right">{{{ Content::money($ent->kprice) or '' }}}</td>
				                  	<td class="text-right">
				                  		<span class="pull-left" {{$colork}}>{{{ $entKamount == $EntAccTran->sum('kcredit') ? Content::money($entKamount) : Content::money($entKamount - $EntAccTran->sum('kcredit')) or '' }}}</span>
	                  				...
	                  				@else
		                  				<span data-toggle="modal" data-target="#loadProjectConfirm" class="btn btn-success btn-xs AddToACcount text-right hidden-print pull-right"
	              							...
	              							data-process_type="pay" 
	              							data-title="{{{ $ent->entrance->name or ''}}}" 
	              							data-book_id="{{$ent->id}}"
	              							data-supplier="" 
...
<!-- 							<th width="120px">City</th> -->
						...
						@foreach($MiscBook as $tour)
							...
							<tr class="container_misc">
				                <td>{{{ Content::dateformat($tour->book_checkin) or '' }}}</td>
				                <!-- <td>{{$pro->province_name}}</td>          -->
						            	...
						            	@foreach($miscService as $misc)
							            	<?php 
						                  	...
						                  	<div class="row col-md-12" style="padding-right: 0px;">
							                  	...
							                  	<label class="col-md-1" style="font-weight: 400;">						                  		
						                  			<span {{$color}}>{{{ $miscAmount - $MiscAccTran->sum('credit') ? Content::money($miscAmount) : Content::money($miscAmount - $MiscAccTran->sum('credit')) or '' }}} </span>
							                  	</label>
							                  	<label class="col-md-1" style="font-weight: 400;">
						                        	    ...
						                        	    @if($MiscJournal['book_amount'] > 0 || $MiscJournal['book_kamount'] > 0)
    					                  				...
    					                  				@else
    					                  					@if($MISCTotalAmount > 0)
    							                  				<span data-toggle="modal" data-target="#loadProjectConfirm" class="btn btn-success btn-xs AddToACcount text-right hidden-print pull-right" 
    					                  							data-type="54" data-kamount="{{{ $misckAmount or '' }}}" 
    					                  							data-amount="{{{ $miscAmount or '' }}}" 
    					                  							data-process_type="pay" 
    					                  							data-title="{{{ $misc->servicetype->name or ''}}}" 
    					                  							data-book_id="{{{ $misc->id or '' }}}"
    					                  							data-supplier="" 
    					                  							data-country="{{{ $tour->country_id or '' }}}">Post
    					                  						</span>
    					                  					@endif
    			                  						@endif
