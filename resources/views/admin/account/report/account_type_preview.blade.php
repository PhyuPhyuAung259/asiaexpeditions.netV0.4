              ...
              <div style="background-color: white; border-radius: 3px; border: solid 1px #ddd; padding: 25px; ">
                <table class="table">
                  <tr style="color: #21919f;">
                    <th style="border-bottom: solid 1px #000000; border-top: none;">{{$acc_name->account_name}}</th>
                  ...
                  @foreach($acc_journal as $key => $acc)                    
                      ...
                      ?>
                    <tr>
                      <td style="display: block;margin-left: 20px;"><a href="{{route('getTrialBalance', ['account_name'=> $acc->account_name_id])}}" target="_blank">{{{ $acc->account_name->account_code or ''}}} - {{{ $acc->account_name->account_name or ''}}}</a></td>

                      <td class="text-right" >{{Content::money($accJournalAmount->sum('credit'))}}</td>
                      <td class="text-right" >{{Content::money($accJournalAmount->sum('debit'))}}</td>
                      <td class="text-right" >{{Content::money($accJournalAmount->sum('kcredit'))}}</td>
                      <td class="text-right" >{{Content::money($accJournalAmount->sum('kdebit'))}}</td>
                    </tr>
                  @endforeach
                  <tr >
